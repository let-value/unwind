...
import {
  ...
  DropdownMenuTrigger,
} from "shadcn-compiled/dropdown-menu.tsx";

import { Example, IconPlaceholder } from "./example.tsx";

const DROPDOWN_MENU_SIDES = [
  "inline-start",
  "left",
  "top",
  "bottom",
...
function DropdownMenuInDialog() {
  return (
    <Example title="In Dialog">
      <div className="grid max-w-md gap-4 rounded-xl border bg-popover p-6 text-popover-foreground">
        <div className="grid gap-2">
          <h2 className="font-medium">Dropdown Menu Example</h2>
  ...
  return (
    <Example title="With Inset">
      <DropdownMenu>
        ...
        <DropdownMenuContent style={{ width: "11rem" }}>
          <DropdownMenuGroup>
...
test("DropdownMenuBasic", async (t) => {
  await render(<DropdownMenuBasic />);
...
for (const side of DROPDOWN_MENU_SIDES) {
  test(`DropdownMenuSide${side}`, async (t) => {
    await render(<DropdownMenuSides />);

    await page.getByRole("button", { name: side.replace("-", " ") }).click();
    const menu = page.getByRole("menu").last();
    await expect.element(menu).toBeVisible();
    await expect.element(menu).toMatchScreenshot(t.task.name);
  });
...
test("DropdownMenuWithIcons", async (t) => {
  await render(<DropdownMenuWithIcons />);
...
test("DropdownMenuWithShortcuts", async (t) => {
  await render(<DropdownMenuWithShortcuts />);
...
test("DropdownMenuWithSubmenu", async (t) => {
  await render(<DropdownMenuWithSubmenu />);
...
test("DropdownMenuWithCheckboxes", async (t) => {
  await render(<DropdownMenuWithCheckboxes />);
...
test("DropdownMenuWithRadio", async (t) => {
  await render(<DropdownMenuWithRadio />);
...
test("DropdownMenuWithCheckboxesIcons", async (t) => {
  await render(<DropdownMenuWithCheckboxesIcons />);
...
test("DropdownMenuWithRadioIcons", async (t) => {
  await render(<DropdownMenuWithRadioIcons />);
...
test("DropdownMenuWithDestructive", async (t) => {
  await render(<DropdownMenuWithDestructive />);
...
test("DropdownMenuWithAvatar", async (t) => {
  await render(<DropdownMenuWithAvatar />);

  await page.getByRole("button", { name: /shadcn@example\.com/i }).click();
  const menu = page.getByRole("menu").last();
  await expect.element(menu).toBeVisible();
  await expect.element(menu).toMatchScreenshot(t.task.name, {
    comparatorOptions: {
...
test("DropdownMenuInDialog", async (t) => {
  await render(<DropdownMenuInDialog />);
...
test("DropdownMenuWithInset", async (t) => {
  await render(<DropdownMenuWithInset />);
...
test("DropdownMenuComplex", async (t) => {
  await render(<DropdownMenuComplex />);
