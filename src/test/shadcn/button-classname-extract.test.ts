...
describe("extractClassNameStringsFromFile", () => {
  ...
  it("generates stable transform targets for the shadcn button fixture", async () => {
    const extracted = await extractClassNameStringsFromFile(buttonFilePath);
  ...
  it("keeps button target selectors aligned with the extracted class strings", async () => {
    const extracted = await extractClassNameStringsFromFile(buttonFilePath);
    const targets = createTransformTargets(extracted);
    ...
    assert.equal(
      targetBySelector.get("button"),
      "group/button inline-flex shrink-0 items-center justify-center rounded-md border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
    );
    assert.equal(
      targetBySelector.get("button-variant-outline"),
      "border-border bg-background shadow-xs hover:bg-muted hover:text-foreground aria-expanded:bg-muted aria-expanded:text-foreground dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
    );
    assert.equal(targetBySelector.get("button-size-icon"), "size-9");
    assert.equal(targetBySelector.get("button-size-icon-lg"), "size-10");
  });

  it("compiles button fixture into local and hoisted global css", async (t) => {
    const extracted = await extractClassNameStringsFromFile(buttonFilePath);
    const targets = createTransformTargets(extracted);
    ...
    assert.ok(result.localCss.includes(".button"));
    assert.ok(result.localCss.includes(".button-variant-default"));
    assert.ok(result.localCss.includes(".button-size-icon-lg"));
    assert.ok(result.localCss.includes(":global(.dark)"));
    assert.ok(!result.localCss.includes(":root"));
    ...
    assert.ok(!result.localCss.includes("{}"));
    assert.ok(!result.globalCss.includes("{}"));

    t.assert.snapshot(snapshotCss(result.localCss));
    t.assert.snapshot(snapshotCss(result.globalCss));
  });
});
